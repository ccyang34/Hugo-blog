...
def save_to_hugo_blog(title, content, date):
    """保存分析报告到Hugo博客"""
    print(f"\n{'='*20} HUGO BLOG {'='*20}")
    print(f"正在保存报告到 Hugo 博客...")
...
---
*数据来源: AkShare | AI 分析: DeepSeek | 自动生成于 {date.strftime('%Y-%m-%d %H:%M:%S')}*
"""
        ...
        print(f"✅ Hugo 博客文章已保存: {filepath}")
        print(f"📝 文章标题: {title}")
        print(f"📅 发布日期: {date.strftime('%Y-%m-%d %H:%M:%S')}")
        
        return filepath
        
    except Exception as e:
        print(f"❌ 保存 Hugo 博客失败: {e}")
        return None

def create_hugo_blog_index():
    """创建Hugo博客的索引页面"""
    try:
        index_path = os.path.join(HUGO_BLOG_DIR, "README.md")
        
        index_content = """# 油脂期货分析博客
...
"""
        ...
        print(f"✅ Hugo 博客索引页面已创建: {index_path}")
        
    except Exception as e:
        print(f"⚠️ 创建索引页面失败: {e}")
...
**分析时间**: {beijing_time.strftime('%Y-%m-%d %H:%M')} (北京时间)
...
"""
    ...
    final_report = report_header + ai_report
    
    # 8. 保存分析报告到Hugo博客
    print(f"\n[{beijing_time.strftime('%H:%M:%S')}] 正在保存分析报告...")
    
    # 创建Hugo博客索引
    create_hugo_blog_index()
    
    # 保存到Hugo博客
    hugo_file_path = save_to_hugo_blog(report_title, final_report, beijing_time)
    
    if hugo_file_path:
        print(f"✅ 分析报告已成功保存到 Hugo 博客: {hugo_file_path}")
        print(f"🎉 油脂期货分析任务完成！")
    else:
        print("❌ 保存 Hugo 博客失败，但分析任务已完成")
        
    print(f"\n📋 分析摘要:")
    print(f"   - 数据获取: ✅ 成功")
    print(f"   - AI分析: ✅ 完成") 
    print(f"   - Hugo博客: {'✅ 保存成功' if hugo_file_path else '❌ 保存失败'}")
    print(f"   - 分析时间: {beijing_time.strftime('%Y-%m-%d %H:%M:%S')}")
